...
function _venv_deactivate() {
    # Works whether or not the deactivate function was inherited
    if [[ -n "$VIRTUAL_ENV" ]]; then
        if declare -f deactivate > /dev/null 2>&1; then
            deactivate
        else
            # Manual cleanup for inherited/orphaned venv state (e.g. new tmux pane)
...
function auto_venv_activate() {
    ...
    # Switch only if a different venv is currently active
